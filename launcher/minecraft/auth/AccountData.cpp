...
bool AccountData::resumeStateFromV3(QJsonObject data)
{
    ...
    // Keep a stable per-account id across restarts (used as the on-disk filename).
    // Older data without this field keeps the id generated by the constructor.
    if (data.contains("internalId") && data.value("internalId").isString()) {
        internalId = data.value("internalId").toString();
    }
